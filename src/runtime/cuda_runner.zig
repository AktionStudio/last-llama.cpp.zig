...
fn generate(a: std.mem.Allocator, model: *llama.Model, req: Request, out: *Result) !void {
    ...
    if (req.cancel_after_abort_checks != null) return error.UnsupportedInDecodeCancellation;
    const start = c.ggml_time_ms();
    defer out.elapsed_ms = c.ggml_time_ms() - start;
    if (!std.mem.eql(u8, req.backend, "cuda")) return error.UnsupportedBackend;
    ...
    // CUDA has no supported in-decode abort callback in this pinned engine.
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len == 2 and std.mem.eql(u8, args[1], "--inspect-runtime")) {
        ...
        const request = Request{ .id = "runtime-inspection", .backend = "cuda", .gpu_layers = 1 };
        const selection = try support.resolveDevice(request, .cuda);
        const inspection = try support.inspectRuntime(inspection_allocator, init.io, .cuda, selection);
    ...
    if (args.len < 2 or args.len > 5) return error.ExpectedModelAndRequestsFile;
    ...
    var selection = try support.resolveDevice(requests[0], .cuda);
    mp.devices = &selection.devices;
    const gpu_layers = if (args.len == 5) try std.fmt.parseInt(i32, args[4], 10) else @as(i32, @intCast(requests[0].gpu_layers));
    if (gpu_layers <= 0 or gpu_layers > 1000 or gpu_layers != requests[0].gpu_layers) return error.InvalidGpuLayers;
    mp.n_gpu_layers = gpu_layers;
    const device = selection.devices[0];
    std.debug.print("RUNTIME_CUDA_SELECTED id={s} description={s} requested_layers={d}\n", .{ selection.device_id, selection.description, gpu_layers });
    ...
    c.ggml_backend_dev_memory(device, &free_bytes, &total_bytes);
    std.debug.print("LAB_GPU_MEMORY baseline free={d} total={d}\n", .{ free_bytes, total_bytes });
    for (0..cycles) |cycle| {
        ...
        c.ggml_backend_dev_memory(device, &free_bytes, &total_bytes);
        std.debug.print("LAB_GPU_MEMORY loaded cycle={d} free={d} total={d}\n", .{ cycle, free_bytes, total_bytes });
        defer {
            model.deinit();
            c.ggml_backend_dev_memory(device, &free_bytes, &total_bytes);
            std.debug.print("LAB_GPU_MEMORY unloaded cycle={d} free={d} total={d}\n", .{ cycle, free_bytes, total_bytes });
            std.debug.print("LAB_MODEL_UNLOADED cycle={d}\n", .{cycle});
            std.Io.sleep(init.io, .fromMilliseconds(100), .awake) catch {};
        }
        const effective_offload = try offload.verify(.cuda, requests[0].gpu_layers, @intCast(model.nLayer() + 1));
        for (requests) |req| {
            const before = gpa.total_requested_bytes;
            {
                ...
                var result = Result{
                    ...
                    .backend = "cuda",
                    .cancellation_granularity = "between-decodes",
                    .timeout_semantics = "checked-before-and-after-synchronized-decode",
                };
                ...
                if (rendered) |template| {
                    result.attestation = support.attest(arena.allocator(), init.io, args[1], model, req, .cuda, selection, template.identity, effective_offload) catch |err| blk: {
                        result.status = @errorName(err);
                        result.finish = "error";
                        break :blk null;
                    };
                ...
                if (!std.mem.eql(u8, result.status, "ok")) {
                    // The request-level error is serialized below so callers can
                    // classify it without relying on process stderr text.
                } else if (req.operation == .inspect_model) {
                    ...
                    result.finish = "error";
                } else generate(arena.allocator(), model, req, &result) catch |err| {
                    result.status = @errorName(err);
                    result.finish = "error";
                };
                if (std.mem.eql(u8, result.status, "ok")) try extractFinal(arena.allocator(), req, &result);
