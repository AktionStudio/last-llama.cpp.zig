...
Run every command from the project root. Build products stay under ignored `build/`; the optional NMake/Ninja comparison writes one ignored, isolated evidence directory under `results/build-equivalence/`.
...
The examples below use NMake. Add `-Generator Ninja` to the schema and gate commands when validating an explicit Ninja build.
